<x-admin-layout :title="LabelConstants::EDIT . ' ' . LabelConstants::SKILL">
    <h3 class="text-gray-700 text-3xl font-medium">{{ LabelConstants::EDIT }} {{ LabelConstants::SKILL }}</h3>

    <div class="mt-8">
        <form action="{{ route('admin.skills.update', $skill->id) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="mb-4">
                    ...
                    <x-text-input id="name" name="name" :value="old('name', $skill->name)" required :error="$errors->has('name')" />
                    ...
                    <div class="relative">
                        <x-select id="proficiency" name="proficiency" :error="$errors->has('proficiency')">
                            ...
                            @foreach(OptionConstants::PROFICIENCY_LEVELS as $value => $label)
                                <option value="{{ $value }}" {{ old('proficiency', $skill->proficiency) == $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </x-select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                            <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
                ...
                <div class="mb-4">
                    ...
                    <x-icon-picker name="icon" :value="old('icon', $skill->icon)" />
